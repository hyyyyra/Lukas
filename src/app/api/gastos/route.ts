...
export async function GET(request: Request) {
  try {
    ...
    console.error("[gastos GET] Error:", message)
...
export async function POST(request: Request) {
  try {
    ...
    const { monto, descripcion, id_categoria, finished_at } = await parseMovimientoBody(body, "gasto")
    ...
    const message = error instanceof Error ? error.message : "Error desconocido"
    console.error("[gastos POST] Error:", message)
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!uuid) {
      return NextResponse.json({ message: "UUID del gasto es requerido" }, { status: 400 })
    }
    ...
    const message = error instanceof Error ? error.message : "Error desconocido"
    console.error("[gastos PATCH] Error:", message)

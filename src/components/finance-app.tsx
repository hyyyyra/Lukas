...
import { WelcomeHeader } from "@/src/components/welcome-header"
import { FinancialSummary } from "@/src/components/financial-summary"
import { convertCurrency, type Currency } from "@/src/lib/currency-converter"
import { apiClient } from "@/src/lib/api-client"
import { clearAuthSession, isAuthErrorMessage } from "@/src/lib/auth-session"
      ...
      try {
        ...
        setData((prev) => ({
          ...
          ingresos: Array.isArray(ingresos)
            ? ingresos.map((i: { id?: string; uuid?: string; nombre?: string; monto?: number }) => ({
          ...
          gastos: Array.isArray(gastos)
            ? gastos.map((i: { id?: string; uuid?: string; nombre?: string; monto?: number; pagado?: boolean }) => ({
                id: String(i.id ?? i.uuid ?? ""),
                nombre: i.nombre ?? "",
                monto: Number(i.monto ?? 0),
                pagado: !!i.pagado,
